@php
    ...
    ksort($specialWords); // Tri alphabétique
@endphp

<!DOCTYPE html>
<html>
<head>
    <title>Polish Vocabulary - Linguateka</title>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel="stylesheet" href="{{ asset('css/style-polish-vocabulary-page.css') }}">
    <link rel="icon" href="{{ asset('/others-images/linguateka-favicon.png') }}" type="image/png">
</head>
<body>
    <div class="linguateka-logo">
        <h1>Polish Special Vocabulary</h1>
    </div>

    <div class="expressions-content">
        <p>This page contain special Polish vocabulary who needs to be explain with their translations.</p>
        
        <ul class="vocabulary-list">
          @foreach ($specialWords as $word => $context)
              <li id="{{ strtolower($word) }}">
                  <strong>{{ ucfirst($word) }}</strong> — {{ $context }}
              </li>
          @endforeach
       </ul>
